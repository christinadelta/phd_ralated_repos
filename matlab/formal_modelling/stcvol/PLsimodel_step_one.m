function mout = PLsimodel_step_one(data, probabilities, trials,condtrials, outtype)
...
xR          = 1 - x; % won't probably need this but..
...
for j = 1:mdl
    ...
    for i = 1:nsim
        for cue = 1:nCues
            simdata                                                 = PLsimdata_v1(probabilities, trials, condtrials, outtype);
            outcome(:,i)                                            = simdata.outcomes;     % run pf model fith binary outcomes 
            outcomeR(:,i)                                           = simdata.outcomesR;    % we also want to estimate vals for the red feedback outcomes
            if cue == 1
...
for g = 1:mdl
    ...
    for i = 1:3 % loop over stochasticities
        ...
        mean_stc_lrs(:,i) = [mstc_lrs{1}; mstc_lrs{2}]; % trial-by-trial lrs averaged over simulations (140x3 array)
