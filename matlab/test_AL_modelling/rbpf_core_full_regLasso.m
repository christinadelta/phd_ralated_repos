function NLL_reg = rbpf_core_full_regLasso(x,o,a,parameters,config)
...
s0          = x(4);
v0          = x(5);
lambda      = x(6);         % Regularization strength
...
% calculating Lasso Penalty - only include parameters you're optimizing 
x_temp              = x(1:5);
lasso_penalty       = lambda * sum(abs(x_temp));

% Update NLL to include Ridge penalty
NLL_reg             = NLL + lasso_penalty;
vol                 = estimated(:,1).^-1;
unp                 = estimated(:,2).^-1;
