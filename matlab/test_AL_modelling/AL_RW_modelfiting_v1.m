...
starting_params = [0.2 2];
nll             = lik_modelRW_toSub_v1(starting_params, dat);
...
for sub = 1:nsubs
    ...
    dat                 = alldata{sub,1};
    starting_params     = [0.25 2];

    % define and run the objective function
    obFunc              = @(x) lik_modelRW_toSub_v1([x(1), x(2)],dat);
    ...
    fit_PP(:,:,sub)     = dat.allPs;
    fit_Qval(:,:,sub)   = dat.Qvals;
    fit_correct(:,sub)  = dat.correct;

end % end of subject loop

%% get average of Q values and choice probs and plot 

av_PPs                  = mean(fit_PP,3);           % average over subjects
av_Qvals                = mean(fit_Qval, 3);  
fit_perf                = mean(fit_correct,"all");  % model performance 
sub_perf                = mean(allsub_mAL,2);       % subject performance 
...
subplot(1,2,1); % 1-row, 2-column grid, 1st subplot
...
subplot(1,2,2); % 1-row, 2-column grid, 2nd subplot
...
sgtitle('Normalized Distributions of Learning Rate and Inverse Temperature Parameters');
