function [val,vol,unp,lr,unc] = rbpf_coreb(x,o,a,parameters,config)
...
% Inputs: 
          % 1) parameters structure: 
                % 1st: lambda_s, -- free (see x(1))
                % 2nd: lambda_v, -- free (see x(2))
                % 3rd: beta,
                % 4rd: s0,
                % 5th: v0,
                % 6th: nparticles
...
for t=1:size(o(:,1),1)   
    ...
    for c = 1:size(o,2)   
        ...
        unc(t,c)          = pf.weights*w';
        lr(t,c)         = pf.weights*k';

    end % end of cues/options loop

%%%%% WILL UPDATE THIS PART TO ALSO GET MODEL PERFORMANCE (FOR COMPARISON
%%%%% WITH HUMANS)
